...
def pend_feat_extract(states, cfg):
    ...
    if use_norm:
        ...
        mu = np.array(
            [0.7473041415214539, -0.04867269843816757, 0.11523590981960297])
        std = np.array(
            [0.5509276390075684, 0.36831530928611755, 1.6884348392486572])
    ...
    if feat_selection == "first":
        ...
        # find features
        second_degree = [
            x * y for x, y in combinations_with_replacement(curr_point, 2)
        ]
...
def acrobot_feat_extract(states, cfg):
    ...
    if use_norm:
        ...
        mu = np.array([
            0.5131067037582397,
            0.0005170440417714417,
            0.1857973039150238,
            0.0010799641022458673,
            -0.029419755563139915,
            0.7311022877693176,
        ])
        std = np.array([
            0.5434738993644714,
    ...
    if feat_selection == "first":
        ...
        # manual
        feats.append(states_std[0] * states_std[2] -
                     states_std[1] * states_std[3])
        feats.append(states_std[0])
        feats.append(states_std[2]**2)
        feats.append(states_std[1]**2)
